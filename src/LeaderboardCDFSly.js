...
function LeaderboardCDFSLY() {
    ...
    const loadLeaderboard = async () => {
        try {
            ...
            const snapshotsKey = `cdf_sly_ranks_snapshots_${leaderboard_id}`;
            const rankSnapshots = JSON.parse(localStorage.getItem(snapshotsKey) || '{}');
            const newIndicators = {};
            let hasNewChanges = false;
            const changedTeams = new Set();
            const prevRunIndicatorsKey = `cdf_sly_prev_run_indicators_${leaderboard_id}`;
...
export default LeaderboardCDFSLY